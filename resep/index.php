<?php
require_once '../config/cek_login.php';require_once '../config/koneksi.php';require_once '../includes/fungsi_gizi.php';$id_user = $_SESSION['id_user'];$stmt = mysqli_prepare($koneksi, "    SELECT r.*, kr.nama_kategori     FROM resep_pribadi r    LEFT JOIN kategori_resep kr ON r.id_kategori = kr.id    WHERE r.id_user = ?    ORDER BY r.created_at DESC");mysqli_stmt_bind_param($stmt, 'i', $id_user);mysqli_stmt_execute($stmt);$result = mysqli_stmt_get_result($stmt);$resep_list = mysqli_fetch_all($result, MYSQLI_ASSOC);mysqli_stmt_close($stmt);
?>
<!DOCTYPE html><html lang="id"><head>    <meta charset="UTF-8">    <meta name="viewport" content="width=device-width, initial-scale=1.0">    <link rel="icon" type="image/png" href="../assets/images/favicon.png">    <title>Resep Saya — Rasa dan Gizi</title>    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">    <script src="https://cdn.tailwindcss.com"></script></head><body class="bg-[#FAF7F2] text-[#2C2620] font-sans antialiased min-h-screen">
<?php $base_path = '../'; $active_page = 'resep'; require __DIR__ . '/../includes/partials/navbar.php'; ?>
<div class="max-w-6xl mx-auto px-6 py-8">    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 mb-8 sm:mb-10">        <div>            <span class="text-[#A3492D] text-[11px] md:text-[12px] tracking-[0.15em] uppercase block mb-1">Dashboard</span>            <h1 class="font-serif text-2xl sm:text-3xl text-[#2C2620] font-normal">Resep Saya</h1>        </div>        <a href="tambah.php" class="w-full sm:w-auto py-2 px-4 sm:py-2.5 sm:px-5 border border-[#A3492D] bg-[#A3492D] text-white text-[12px] sm:text-[13px] tracking-[0.1em] uppercase hover:bg-[#8B3D25] hover:-translate-y-0.5 shadow-[0_6px_14px_rgba(163,73,45,0.35)] hover:shadow-[0_8px_22px_rgba(163,73,45,0.45)] transition-all no-underline inline-block text-center">            + Tambah Resep        </a>    </div>    <?php if (empty($resep_list)): ?>        <div class="bg-[#E4DBC8] p-16 text-center">            <p class="text-[#4A4438] text-base mb-5">Belum ada resep. Yuk buat resep pertama kamu!</p>            <a href="tambah.php" class="py-2.5 px-5 border border-[#A3492D] bg-[#A3492D] text-white text-[13px] tracking-[0.1em] uppercase hover:bg-[#8B3D25] hover:-translate-y-0.5 shadow-[0_6px_14px_rgba(163,73,45,0.35)] hover:shadow-[0_8px_22px_rgba(163,73,45,0.45)] transition-all no-underline inline-block">Buat Resep Sekarang</a>        </div>    <?php else: ?>        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">            <?php foreach ($resep_list as $resep):                 $gizi = hitung_gizi_resep_pribadi($resep['id']);
...
?>                    </div>                    <div class="border-t border-[#E4DBC8] px-5 py-3 flex flex-wrap gap-2 text-[12px] md:text-[13px]">                        <a href="detail.php?id=<?= $resep['id'] ?>" class="py-1.5 px-2 md:px-3 border border-[#A3492D] bg-white text-[#A3492D] no-underline hover:bg-[#A3492D] hover:text-white hover:-translate-y-0.5 shadow-[0_4px_10px_rgba(0,0,0,0.14)] hover:shadow-[0_7px_16px_rgba(0,0,0,0.2)] transition-all">Detail</a>                        <a href="edit.php?id=<?= $resep['id'] ?>" class="py-1.5 px-2 md:px-3 border border-[#D1C4B0] bg-white text-[#6B6154] no-underline hover:bg-[#F5F0E8] hover:-translate-y-0.5 shadow-[0_4px_10px_rgba(0,0,0,0.14)] hover:shadow-[0_7px_16px_rgba(0,0,0,0.2)] transition-all">Edit</a>                        <a href="hapus.php?id=<?= $resep['id'] ?>" class="py-1.5 px-2 md:px-3 border border-[#D1C4B0] bg-white text-[#6B6154] no-underline hover:bg-[#F5F0E8] hover:-translate-y-0.5 shadow-[0_4px_10px_rgba(0,0,0,0.14)] hover:shadow-[0_7px_16px_rgba(0,0,0,0.2)] transition-all" onclick="confirmHapus(event, this)">Hapus</a>                    </div>                </div>            <?php endforeach;
?>        </div>    <?php endif;
?>
</div>
